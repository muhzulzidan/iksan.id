...
const CheckoutKelas = ({ userData }: { userData: UserData }) => {
    ...
    const kelasSlug = searchParams.get('kelas')
    ...
    useEffect(() => {
        if (kelasSlug) {
            // Fetch the kelas data from Contentful
            axios.get(`/api/kelas?slug=${kelasSlug}`)
                .then(response => {
